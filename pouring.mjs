...
export function realCup(kind,config){
 ...
 const id='real-'+kind;
 const body='M12 20 Q50 8 88 20 L73 115 Q50 122 27 115 Z';
 ...
 <rect x="8" y="14" width="84" height="110" fill="url(#${id}-espresso)"/>
 ${!brew?`<image href="assets/latte-noice-photo.png" y="-12" width="100" height="139" preserveAspectRatio="none" class="real-milk-photo"/>`:''}
 ...
 <g class="ice-rise ${milk?'rising':''}" style="--ice-rise:${kind==='finish'?'-58px':'0px'}">${Array.from({length:count},(_,i)=>`<g class="real-ice ${ice?'falling':''}" style="--drop-delay:${i*.24}s"><g transform="translate(${30+i%3*13} ${78+Math.floor(i/3)*11}) rotate(${i%2?18:-17} 7 7)"><path d="M2 0L13 1L16 11L12 16L1 14L0 4Z" fill="url(#${id}-ice)" stroke="#fff9" stroke-width=".5"/><path d="M2 2L11 3L13 12M3 4L4 11L11 13" fill="none" stroke="#fff8" stroke-width=".8"/></g></g>`).join('')}</g>
 ${count===7&&(milk||kind==='finish')?`<image href="assets/latte-photo.png" y="-12" width="100" height="139" preserveAspectRatio="none" class="real-iced-photo ${milk?'ice-reveal':''}"/>`:''}
 ${(milk||kind==='finish')?`<ellipse class="milk-surface ${milk?'arriving':''}" cx="50" cy="19" rx="36" ry="5" fill="#cfa273" stroke="#edd2a6" stroke-width=".5"/>`:''}
 </g>
